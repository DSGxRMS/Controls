...
class PathFollower(Node):
    def __init__(self):
        ...
        self.declare_parameter('path_topic', '/path_window')   # <-- matches the path publisher
        ...
        self.best_effort = bool(self.get_parameter('qos_best_effort').value)

        # Unified QoS (BEST_EFFORT everywhere as requested)
        sub_qos = QoSProfile(
        ...
        self.create_subscription(Float32MultiArray, self.path_topic, self._path_cb, sub_qos)
    ...
    def _path_cb(self, msg: Float32MultiArray):
        ...
        if len(arr) % 2 != 0 or len(arr) < 2:
            self.get_logger().warn("Invalid path message length (expected even number of values >= 2)")
            return

        xs = arr[0::2]
        ys = arr[1::2]

        # Use your utilities for preprocessing
